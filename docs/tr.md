...
3. Получение подборки фильмов
4. Отправка свайпа (голосования)
5. Обработка совпадения (Match Found)
6. Восстановление сессии при потере соединения
7. Завершение сессии
8. Ошибочные сценарии:

   
| Ситуация                                      | Действие системы                                              |
| --------------------------------------------- | ------------------------------------------------------------- |
| Пользователь вводит неверный код сессии       | Возврат `404 Session not found`                               |
| Потеря соединения во время голосования        | Голос сохраняется локально и отправляется при восстановлении  |
| Сбой одного сервиса (Match Service, Film API) | Ретрай через очередь сообщений / резервный API                |
| Пользователь покидает сессию                  | Остальные получают событие `user_left`, состояние обновляется |

---

## План разработки и тестирования
| Этап                               | Цель                                                           | Основные артефакты                                 |
| ---------------------------------- | -------------------------------------------------------------- | -------------------------------------------------- |
| **1. Анализ и проектирование**     | проектирование API и БД | архитектурная схема              |
| **2. Разработка прототипа (MVP)**  | Создание минимального жизнеспособного продукта                 | Клиент + базовый backend, подключение по WebSocket |
| **3. Реализация основных функций** | Голосование, синхронизация, матчинг, восстановление сессии     | Полный backend, реалтайм-коммуникация              |
| **4. Интеграция и тестирование**   | Тестирование взаимодействия модулей и UI                       | Тест-план, отчёты о тестах                         |
| **5. Оптимизация и деплой**        | Улучшение производительности, финальные проверки, релиз        | CI/CD pipeline, Docker-окружение, документация     |

### backend

| Сервис                 | Функции                                   | 
| ---------------------- | ----------------------------------------- | 
| Auth Service           | JWT, гостевая авторизация                 |
| Session Service        | CRUD сессий, участники, reconnect         |
| Realtime Service       | рассылка событий               |
| Match Service          | Логика совпадений, проверка голосов       |
| Recommendation Service | Подбор фильмов, кэширование               | 
| Storage Service        | Работа с PostgreSQL и Redis               | 
| API Gateway            | Маршрутизация, логирование, rate limiting | 

### frontend

| Подзадача           | Описание                                         |
| ------------------- | ------------------------------------------------ | 
| UI макеты           | Проектирование экранов (главная, карточки, матч) | 
| Навигация           | Реализация маршрутизации между экранами          | 
| Авторизация         | Реализация входа гостя и пользователя            |
| Сессия              | Создание / подключение к сессии по коду          |
| Голосование         | Реализация свайпов и отправка событий            | 
| Синхронизация       | Подключение WebSocket, обработка событий         |
| Локальное хранилище | Списки "Просмотрено", "Позже"                    | 
